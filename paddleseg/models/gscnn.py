...
class GSCNNHead(nn.Layer):
    ...
    def __init__(self, num_classes, backbone_indices, backbone_channels,
        ...
        self.dsn1 = nn.Conv2D(backbone_channels[backbone_indices[1]],
                              1,
                              kernel_size=1)
        self.dsn2 = nn.Conv2D(backbone_channels[backbone_indices[2]],
                              1,
                              kernel_size=1)
        self.dsn3 = nn.Conv2D(backbone_channels[backbone_indices[3]],
                              1,
                              kernel_size=1)
        ...
        self.aspp = ASPPModule(aspp_ratios=aspp_ratios,
                               in_channels=backbone_channels[-1],
                               out_channels=aspp_out_channels,
                               align_corners=self.align_corners,
                               image_pooling=True)

        self.decoder = deeplab.Decoder(num_classes=num_classes,
                                       in_channels=backbone_channels[0],
                                       align_corners=self.align_corners)

    def forward(self, x, feat_list, s_input):
        input_shape = paddle.shape(x)
        m1f = F.interpolate(s_input,
                            input_shape[2:],
                            mode='bilinear',
                            align_corners=self.align_corners)
        ...
        s1 = F.interpolate(self.dsn1(l1),
                           input_shape[2:],
                           mode='bilinear',
                           align_corners=self.align_corners)
        s2 = F.interpolate(self.dsn2(l2),
                           input_shape[2:],
                           mode='bilinear',
                           align_corners=self.align_corners)
        s3 = F.interpolate(self.dsn3(l3),
                           input_shape[2:],
                           mode='bilinear',
                           align_corners=self.align_corners)
